...
int main(){
  ...
  cin.tie(NULL);
  TC {
    ll desired, k, coin_one, coin_multiple, total = 0;
    cin >> desired >> k >> coin_one >> coin_multiple;
    while (coin_multiple--){
      if (total + k <= desired){total+= k;}
      else {break;}
    }
    if (coin_one + total >= desired){ o(0);}
    total = total + coin_one;
    ll needed = desired - total;
    ll res = (needed/k) + (needed % k);
    if (coin_one > k - (needed % k)){ res = min(res, ((needed/k)) + 1);}
    o(res);
  }
}
